...
int main(void) {
    ...
    // Take the user's bubble tea order | step1
    struct bubble_tea order = take_order();

    // Check available stock and end the program if insufficient stock | step2
    if (check_stock(inventory, order) == 1) {
        return 1;
    }

    // Calculate the cost | step3
    double total_cost = calculate_cost(order);

    // Print the final order and cost | step4
    print_order(order, total_cost);

    // Update the stock after fulfilling the order | step5
    inventory = update_stock(order, inventory);

    // Print the remaining inventory | step6
